...
knn_tuning_enose_Pseud
knn_tuning_enose_TVC
```

```{r}
#Run knn model for TVC bacteria counts, 1 iteration
knn_model_HPLC_TVC <- run_reg_model(AllData = AllData_HPLC_counts, predictor = AllData_HPLC_counts$TVC, bacteria = "TVC", perc_pred = .7, times = 1, 
                      formula = TVC~., method = "knn", tuneGrid = expand.grid(k = 1:20))

summary(knn_model_HPLC_TVC$model.fit)
knn_model_HPLC_TVC$predict.model
knn_model_HPLC_TVC$RMSE.val
knn_model_HPLC_TVC$plt_RMSE

```

```{r}
#Run knn model for TVC bacteria counts, 1 iteration
knn_model_HPLC_TVC <- run_reg_model_iteration(AllData = AllData_HPLC_counts, predictor = AllData_HPLC_counts$TVC, bacteria = "TVC", perc_pred = .7, times = 1, 
                      formula = TVC~., method = "knn", tuneGrid = expand.grid(k = 1:20), preProcess = c("center","scale"))
