...
#HPLC data
knn_var_importance_HPLC <- var_importance(AllData_HPLC_sens, AllData_HPLC_sens$sensory, .7, 1, sensory~., method = "knn", tuneGrid = expand.grid(k = 1:20))

#Importance plot
importance_plot(knn_var_importance_HPLC, "HPLC, knn")

#Enose data
#HPLC data
knn_var_importance_enose <- var_importance(AllData_enose_sens, AllData_enose_sens$sensory, .7, 1, sensory~., method = "knn", tuneGrid = expand.grid(k = 1:20))

#Importance plot
importance_plot(knn_var_importance_enose, "enose, knn")
```
Variable importance for svm
```{r}
#Calculate the importance for svm
#HPLC data
svm_var_importance_HPLC <- var_importance(AllData_HPLC_sens, AllData_HPLC_sens$sensory, .7, 1, sensory~., method = "svmRadial", tuneGrid = NULL)

#Importance plot
importance_plot(svm_var_importance_HPLC, "HPLC, svm")

#Enose data
svm_var_importance_enose <- var_importance(AllData_enose_sens, AllData_enose_sens$sensory, .7, 1, sensory~., method = "svmRadial", tuneGrid = NULL)

#Importance plot
importance_plot(svm_var_importance_enose, "enose, svm")
```
...
knn_tuning_HPLC_Pseud <- run_reg_model_tuning(AllData = AllData_HPLC_counts, perc_pred = .7, times = 1, method = "knn", bacteria = "Pseudomonads", 
                        tuneGrid = expand.grid(k = 1:20), preProcess = c("center","scale"))

#Apply tuning to TVC, HPLC
knn_tuning_HPLC_TVC <- run_reg_model_tuning(AllData = AllData_HPLC_counts, perc_pred = .7, times = 1, method = "knn", bacteria = "TVC", 
                        tuneGrid = expand.grid(k = 1:20), preProcess = c("center","scale"))
...
```{r}
#Apply tuning to Pseudomonads, HPLC
knn_tuning_enose_Pseud <- run_reg_model_tuning(AllData = AllData_enose_counts, perc_pred = .7, times = 1, method = "knn", bacteria = "Pseudomonads", 
                        tuneGrid = expand.grid(k = 1:20), preProcess = c("center","scale"))

#Apply tuning to TVC, HPLC
knn_tuning_enose_TVC <- run_reg_model_tuning(AllData = AllData_enose_counts, perc_pred = .7, times = 1, method = "knn", bacteria = "TVC", 
                        tuneGrid = expand.grid(k = 1:20), preProcess = c("center","scale"))
...
```{r, warning=FALSE}
run_reg_HPLC_TVC <- run_reg_model_iteration(AllData = AllData_HPLC_counts, perc_pred = .7, times = 100, method = "knn", bacteria = "TVC", 
                        tuneGrid = expand.grid(k = 1:20), preProcess = c("center","scale"))
#Mean of RMSE for 100 iterations
run_reg_HPLC_TVC$mean_RMSE
```
knn model to HPLC, Pseudomonads, 100 iterations
```{r, warning=FALSE}
run_reg_HPLC_Pseudo <- run_reg_model_iteration(AllData = AllData_HPLC_counts, perc_pred = .7, times = 100, method = "knn", bacteria = "Pseudomonads", 
                        tuneGrid = expand.grid(k = 1:20), preProcess = c("center","scale"))
#Mean of RMSE for 100 iterations
run_reg_HPLC_Pseudo$mean_RMSE
```

knn model to enose, TVC, 100 iterations
```{r, warning=FALSE}
run_reg_enose_TVC <- run_reg_model_iteration(AllData = AllData_enose_counts, perc_pred = .7, times = 100, method = "knn", bacteria = "TVC", 
                        tuneGrid = expand.grid(k = 1:20), preProcess = c("center","scale"))
#Mean of RMSE for 100 iterations
run_reg_enose_TVC$mean_RMSE
```

knn model to enose, Pseudomonads, 100 iterations
```{r, warning=FALSE}
run_reg_enose_Pseudo <- run_reg_model_iteration(AllData = AllData_enose_counts, perc_pred = .7, times = 100, method = "knn", bacteria = "Pseudomonads", 
                        tuneGrid = expand.grid(k = 1:20), preProcess = c("center","scale"))
#Mean of RMSE for 100 iterations
run_reg_enose_Pseudo$mean_RMSE
```
...
#Apply tuning to Pseudomonads, HPLC
```{r}
rf_tuning_HPLC_Pseud <- run_reg_model_tuning(AllData = AllData_HPLC_counts, perc_pred = .7, times = 1, method = "rf", bacteria = "Pseudomonads", 
                        tuneGrid = expand.grid(mtry = 2:19), preProcess = c("center","scale"))

#Apply tuning to TVC, HPLC
rf_tuning_HPLC_TVC <- run_reg_model_tuning(AllData = AllData_HPLC_counts, perc_pred = .7, times = 1, method = "knn", bacteria = "TVC", 
                        tuneGrid = expand.grid(mtry = 2:19), preProcess = c("center","scale"))
...
```{r}
#Apply tuning to Pseudomonads, HPLC
rf_tuning_enose_Pseud <- run_reg_model_tuning(AllData = AllData_enose_counts, perc_pred = .7, times = 1, method = "knn", bacteria = "Pseudomonads", 
                        tuneGrid = expand.grid(mtry = 2:9), preProcess = c("center","scale"))

#Apply tuning to TVC, HPLC
rf_tuning_enose_TVC <- run_reg_model_tuning(AllData = AllData_enose_counts, perc_pred = .7, times = 1, method = "knn", bacteria = "TVC", 
                        tuneGrid = expand.grid(mtry = 2:9), preProcess = c("center","scale"))
...
```{r}
#rf for HPLC, TVC
tun_rf_HPLC_tvc <- run_reg_model_iteration(AllData = AllData_HPLC_counts, perc_pred = .7, times = 1, method = "rf", bacteria = "TVC", 
                        tuneGrid = expand.grid(mtry = 2:19))

#Display best tune for random forest
tun_rf_HPLC_tvc$model.fit$bestTune
tun_rf_HPLC_tvc$model.fit
tun_rf_HPLC_tvc$mean_RMSE
```

```{r}
#rf for HPLC, Pseudomonads
tun_rf_HPLC_Pseudo <- run_reg_model_iteration(AllData = AllData_HPLC_counts, perc_pred = .7, times = 1, method = "rf", bacteria = "Pseudomonads", 
                        tuneGrid = expand.grid(mtry = 2:19))

#Display best tune for random forest
tun_rf_HPLC_Pseudo$model.fit$bestTune
tun_rf_HPLC_Pseudo$model.fit
tun_rf_HPLC_Pseudo$mean_RMSE
```

For enose, apply random forest (rf) regression, tuning for parameter mtry
```{r}
#rf for enose, TVC
tun_rf_enose_tvc <- run_reg_model_iteration(AllData = AllData_enose_counts, perc_pred = .7, times = 1, method = "rf", bacteria = "TVC", 
                        tuneGrid = expand.grid(mtry = 2:9))

#Display best tune for random forest
tun_rf_enose_tvc$model.fit$bestTune
tun_rf_enose_tvc$model.fit
tun_rf_enose_tvc$mean_RMSE
```

```{r}
#rf for enose, Pseudomonads
tun_rf_enose_Pseudo <- run_reg_model_iteration(AllData = AllData_enose_counts, perc_pred = .7, times = 1, method = "rf", bacteria = "Pseudomonads", 
                        tuneGrid = expand.grid(mtry = 2:9))

#Display best tune for random forest
tun_rf_enose_Pseudo$model.fit$bestTune
tun_rf_enose_Pseudo$model.fit
tun_rf_enose_Pseudo$mean_RMSE
```
...
```{r, warning=FALSE}
run_rf_HPLC_TVC <- run_reg_model_iteration(AllData = AllData_HPLC_counts, perc_pred = .7, times = 100, method = "knn", bacteria = "TVC", 
...
```{r, warning=FALSE}
run_rf_HPLC_Pseudo <- run_reg_model_iteration(AllData = AllData_HPLC_counts, perc_pred = .7, times = 100, method = "knn", bacteria = "Pseudomonads", 
                        tuneGrid = expand.grid(k = 2:19))
#Mean of RMSE for 100 iterations
...
```{r, warning=FALSE}
run_rf_enose_TVC <- run_reg_model_iteration(AllData = AllData_enose_counts, perc_pred = .7, times = 100, method = "knn", bacteria = "TVC", 
...
```{r, warning=FALSE}
run_rf_enose_Pseudo <- run_reg_model_iteration(AllData = AllData_enose_counts, perc_pred = .7, times = 100, method = "knn", bacteria = "Pseudomonads", 
                        tuneGrid = expand.grid(k = 2:9))
#Mean of RMSE for 100 iterations
