...
HPLC_sens_svm <- model.run(AllData_HPLC_sens, AllData_HPLC_sens$sensory, 0.7, 100,
...
#Calculate cumulative mean
cum_mean_HPLC_svm <- cumulative.mean.accuracy(HPLC_sens_svm$accuracies)
cum_mean_HPLC_svm
```
...
```{r}
#Run the optimised model for 100 iterations, save the resulting accuracies list
enose_sens_svm <- model.run(AllData_enose_sens, AllData_enose_sens$sensory, 0.7, 100,
          operation = function(trainSet, trainCl, testSet, testCl, kernel_type) {
            #Apply svm with kernel = rbfdot and 
            model_svm <- ksvm(sensory ~ ., data=trainSet,
                                   kernel = "rbfdot", C=1)
...
#Calculate cumulative mean
cum_mean_enose_svm <- cumulative.mean.accuracy(enose_sens_svm$accuracies)
cum_mean_enose_svm
```
...
cum_mean_HPLC_rf <- cumulative.mean.accuracy(rf_accuracies_HPLC)
cum_mean_HPLC_rf
```
...
#Calculate cumulative means accuracies
cum_mean_enose_rf <- cumulative.mean.accuracy(rf_accuracies_enose)
cum_mean_enose_rf
```
...
```{r}
optimal_svm_hplc$bestCrossTable
nb_crossTable <- optimal_svm_hplc$bestCrossTable$t
prop_crossTable <- optimal_svm_hplc$bestCrossTable$prop.tbl


#Vector of number of misclassification
misclassification_nb <- c()
mis_nb_sum <- 0
#Vector of proportion of missclassification
...
for (i in 1:nrow(nb_crossTable)){
  ...
  misclassification_nb <- mis_nb_sum
  misclassification_prop <- mis_prop_sum
}
```
```{r}
HPLC_sens_run_knn <- model.run(AllData_HPLC_sens, AllData_HPLC_sens$sensory, 0.7, 100,
...
HPLC_sens_run_knn$misclassification_nb
HPLC_sens_run_knn$misclassification_prop
```
